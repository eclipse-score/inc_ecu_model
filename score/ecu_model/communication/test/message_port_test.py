...
class TestMessagePort(unittest.TestCase):
    ...
    def test_required_message_port_defaults(self) -> None:
        channel = self._channel()
        port = RequiredMessagePort(name="SpeedPort", channel=channel)
    ...
    def test_queue_sizes_must_be_positive_integers(self) -> None:
        ...
        with self.assertRaises(ValidationError):
            ProvidedMessagePort(name="SpeedPort", channel=channel, max_published_messages=0)

        with self.assertRaises(ValidationError):
            ProvidedMessagePort(name="SpeedPort", channel=channel, max_published_messages=-1)

        with self.assertRaises(ValidationError):
            RequiredMessagePort(name="SpeedPort", channel=channel, max_required_messages=0)

        with self.assertRaises(ValidationError):
            RequiredMessagePort(name="SpeedPort", channel=channel, max_required_messages=-1)
